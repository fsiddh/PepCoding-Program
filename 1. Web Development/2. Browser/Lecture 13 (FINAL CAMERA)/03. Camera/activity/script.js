...
videoRecorder.addEventListener("click", function (e) {
	...
	if (recordState == false) {
		...
		videoRecorder.setAttribute(
			"class",
			"fas fa-stop-circle record-btn record-animation"
		);
	}
...
function startCounting() {
	...
	clearObj = setInterval(function () {
		let seconds =
			timeCount % 60 < 10 ? `0${timeCount % 60}` : `${timeCount % 60}`;
		let minutes =
			timeCount / 60 < 10
				? `0${Number.parseInt(timeCount / 60)}`
				: `${Number.parseInt(timeCount / 60)}`;
		let hours =
			timeCount / 3600 < 10
				? `0${Number.parseInt(timeCount / 3600)}`
				: `${Number.parseInt(timeCount / 3600)}`;
